...
interface UploadResult {
  message: string
  updated: number
  failed: { prn: string; reason: string }[]
  total: number
}

export default function BulkUploadPage() {
  const [selectedType, setSelectedType] = useState<string>('ff180')
              ...
              <div className={`flex items-center gap-3 p-4 rounded-xl border ${
                ...
                <div>
                  ...
                  <p className="text-xs opacity-70 mt-0.5">
                    {result.updated} updated · {result.failed.length} failed · {result.total} total
                  </p>
                </div>
              </div>
                    ...
                    <div className="mt-2 space-y-1 max-h-48 overflow-y-auto">
                      {result.failed.map((f, i) => (
                        <div key={i} className="flex items-center gap-2 p-2 bg-[#1A2540] rounded-lg">
                          ...
                          <span className="text-xs text-[#EEF2FF] font-mono">{f.prn}</span>
